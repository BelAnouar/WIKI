...
<div class="grid lg:grid-cols-2 gap-2 px-4 py-4">
    <?php foreach ($categories as $category) { ?>
        <div class="group mx-2 mt-10 grid max-w-xl grid-cols-12 space-x-8 overflow-hidden rounded-lg border py-8 text-gray-700 shadow transition hover:shadow-lg sm:mx-auto">
            <div class="grid h-16 w-16 ml-4 rounded-lg bg-purple-600 text-white place-items-center uppercase text-2xl font-bold ">
                {Objitem.company?.charAt(0)}
            </div>
            <div class="col-span-11 flex flex-col pr-8 text-left sm:pl-4">
                <h3 class="text-sm text-gray-600"> <?= $category['categoryName'] ?></h3>
            </div>
            ...
            <div class="mt-3 flex flex-col space-y-3 text-sm font-medium text-gray-500 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-2">
                <button id="EditC" value="<?php echo e($category['categoryId']); ?>" class="bg-transparent hover:bg-green-700 text-green-700 font-semibold hover:text-black py-2 px-3 border border-green-500 hover:border-transparent rounded">
                    Edit
                </button>
